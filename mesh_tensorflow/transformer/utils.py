...
def tpu_estimator_model_fn(model_type,
  ...
  def my_model_fn(features, labels, mode, params=None, config=None):
    ...
    for key, x in features.items():
      ...
      if ori_key in sequence_length:
        feature_length = sequence_length[ori_key]
      ...
      else:  # non-sequence features, e.g., weights TODO: debug
        feature_shape = mtf.Shape(ensemble_dims + batch_dims)
        x = tf.reshape(x, feature_shape.to_integer_list)
      if not use_tpu:
