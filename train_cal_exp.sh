...
for model_type in 11B; do
    for train_set in uq_clean_train uq_clean_test; do  # uq_clean_train uq_clean_test
        ...
        python train_cal.py --model temp --mix ${reb_bt_mix} --split dev \
            --score output/exp/${train_set}/dev/${suffix}/${model_type}/uq_ft_margin.txt &> output/exp/model/temp/${model_type}_${train_set}_${suffix}_ft_margin.out
        python train_cal.py --model xgb --mix ${reb_bt_mix} --split dev \
            --score output/exp/${train_set}/dev/${suffix}/${model_type}/uq_ft_margin.txt \
            --inp_perp output/exp/${train_set}/dev/${suffix}_inp/${model_type}/uq_ft_margin.txt \
            --out output/exp/model/xgb/${model_type}_${train_set}_${suffix}_ft_margin.bin &> output/exp/model/xgb/${model_type}_${train_set}_${suffix}_ft_margin.out
    done
done
